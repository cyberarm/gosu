    ...
    def initialize(width, height)
      ...
      @line = lambda{|dx=0,dy=0|
        ...
        Gosu.draw_line(width/2+dx, height*9/10+dy, Gosu::Color::GREEN,
                       width/10+dx, height/10+dy, Gosu::Color::RED)
      }
      @triangle = lambda{|dx=0,dy=0|
        dx ||= 0
        Gosu.draw_triangle(width/10+dx, height/10+dy, Gosu::Color::RED,
                           width*9/10+dx, height/2+dy, Gosu::Color::GREEN,
                           width/2+dx, height*9/10+dy, Gosu::Color::BLUE)
      }
      @quad = lambda{|dx=0,dy=0|
        dx ||= 0
        Gosu.draw_quad(width/10+dx, height/10+dy, Gosu::Color::RED,
                       width*9/10+dx, height/2+dy, Gosu::Color::GREEN,
